...
//static unsigned char Test_Byte1;
...
int main(void)
{
    ...
    while (true)
    {
        //__WFE();
				...
				//nrf_delay_ms(100);
				Read_Sen_datas(0x0f,&Test_Byte,1);
				SEGGER_RTT_printf(0,"G_Sensor ID=:%x\r\n",Test_Byte);
				//Test_Byte=0;
				//write_Sen_datas(0x20, 0x6F);
				
				//nrf_delay_ms(100);
				//Read_Sen_datas(0x20,&Test_Byte1,1);
				//SEGGER_RTT_printf(0,"CTRL_REG1=:%x\r\n",Test_Byte1);
				nrf_delay_ms(10);
				
				Enable_GSensor();
				nrf_delay_ms(10);
				Read_Sen_datas(0x28,Temp,6);
				//Read_Sen_datas(0x28,&Temp[0],1);
				
				SEGGER_RTT_printf(0,"XL:%x ",Temp[0]);
			
				//Read_Sen_datas(0x29,&Temp[1],1);
				SEGGER_RTT_printf(0,"XH:%x\r\n",Temp[1]);
				
				//Read_Sen_datas(0x2A,&Temp[2],1);
				SEGGER_RTT_printf(0,"YL:%x ",Temp[2]);
				
				//Read_Sen_datas(0x2B,&Temp[3],1);
				SEGGER_RTT_printf(0,"YH:%x\r\n",Temp[3]);
				
				//Read_Sen_datas(0x2C,&Temp[4],1);
				SEGGER_RTT_printf(0,"ZL:%x ",Temp[4]);
				
				//Read_Sen_datas(0x2D,&Temp[5],1);
				SEGGER_RTT_printf(0,"ZH:%x\r\n",Temp[5]);			
				//Disable_GSensor();
				x=(Temp[0]>>4)|(WORD)Temp[1]<<4;
				y=(Temp[2]>>4)|(WORD)Temp[3]<<4;
				z=(Temp[4]>>4)|(WORD)Temp[5]<<4;
				X=x;
				Y=y;
				Z=z;
