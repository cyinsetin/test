...
#define BOOTLOADER_DFU_START  0xb0
/*************************************/

#define IS_SRVC_CHANGED_CHARACT_PRESENT 0                                           /**< Include the service_changed characteristic. If not enabled, the server's database cannot be changed for the lifetime of the device. */

#define DEVICE_NAME                     "CT-003V022"                               /**< Name of device. Will be included in the advertising data. */
...
#define SERIAL_NUMBER										"20161105CY003001"
 ...
 */
static void send_data_check_sum(uint8_t * p_data,uint8_t * sum1,uint8_t * sum2)
{
...
static void RX_data_check_sum(uint8_t * p_data,uint8_t * sum1,uint8_t * sum2)
{
	uint8_t i,len,ck_a,ck_b;
	len=p_data[4];
	ck_a=ck_b=0;
...
//    static uint8_t index = 0;	
	...
	*/
			...
			if (!err_f) {
										...
										if ((ck_a!=p_data[length-2])||(ck_b!=p_data[length-1])) err_f=true;
									}
			/*
			if (err_f) {
										bt_rx_len=8;
			  ...
			  cmd=p_data[3];
				switch (cmd) {
											case 0x00:	//Get Device S/N
														...
														send_data_check_sum(SN,&BT_TxBuf[bt_tx_len-1],&BT_TxBuf[bt_tx_len-2]);
...
{
	//uint32_t err_code;
	switch(Hand_activity_mode)			//处理工作模式
	{
		case Time_Mode_index:					//0:Time Mode
					if(Minute_Hand_One_Step_flg==1)
					{
						Minute_Hand_Clockwise(&Minute_Hand_Position,&Minute_Hand_Clockwise_Step_Cnt);			//分针顺时针处理
...
//    uint8_t ck_a,ck_b;
    ...
    // Initialize.
    APP_TIMER_INIT(APP_TIMER_PRESCALER, APP_TIMER_OP_QUEUE_SIZE, false);			//APP定时器初始化
    uart_init();															//Uart串口初始化，端口配置等
    buttons_leds_init(&erase_bonds);					//按键和LED初始化配置
    ble_stack_init();													//蓝牙协议栈初始化
    gap_params_init();												//
    services_init();													//设备信息初始化
    advertising_init();												//
    conn_params_init();												//
    adc_config();															//
	  nrf_adc_start();													//
