...
*/
    ...
    OLED_WrByte(0x00,OLED_WR_CMD);//---set low column address
    OLED_WrByte(0x20,OLED_WR_CMD);//---set high column address
		
    OLED_WrByte(0x00,OLED_WR_CMD);//--set start line address  Set Mapping RAM Display Start Line (0x00~0x3F)
		
    OLED_WrByte(0xB0,OLED_WR_CMD);//--set contrast control register
    ...
    OLED_WrByte(0xA6,OLED_WR_CMD);//Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
		
    OLED_WrByte(0xA8,OLED_WR_CMD);//--set normal display
		OLED_WrByte(0x1F,OLED_WR_CMD);//--set multiplex ratio(1 to 64)
				
    OLED_WrByte(0xC8,OLED_WR_CMD);//--1/64 duty
    ...
    OLED_WrByte(0x1F,OLED_WR_CMD);//Set Pre-Charge as 15 Clocks & Discharge as 1 Clock
		//OLED_WrByte(0x22,OLED_WR_CMD);
    OLED_WrByte(0xDA,OLED_WR_CMD);//--set com pins hardware configuration
		
    OLED_WrByte(0x00,OLED_WR_CMD);
    OLED_WrByte(0xDB,OLED_WR_CMD);//--set vcomh
		
    OLED_WrByte(0x40,OLED_WR_CMD);//Set VCOM Deselect Level
		
    OLED_WrByte(0x8D,OLED_WR_CMD);//-Set Page Addressing Mode (0x00/0x01/0x02)
    OLED_WrByte(0x14,OLED_WR_CMD);//
